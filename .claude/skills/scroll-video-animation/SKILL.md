...
- Poll `job_status` (`sync:true`, ~60–180s), then download the mp4 into the scratchpad (never commit it). Spot-check 3 frames (`select='eq(n,0)+eq(n,60)+eq(n,119)'`) before extracting: first frame ≈ start image, mid frame coherent, last ≈ end image.
